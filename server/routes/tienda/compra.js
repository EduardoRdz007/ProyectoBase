...
app.post('/', async(req, res) => {
    ...
    try {
        ...
        const animalitoDisponible = await TiendaModel.findOne({ _id: idTienda, arrAnimalitos: compra.idAnimalito });

        if (animalitoDisponible == null) {
            return res.status(404).send({
                estatus: '404',
                err: true,
                msg: 'Error: no se encontro el animalito o ya esta vendido.',
                cont: 0
            });
        } else {
            const rmvAnimalito = await TiendaModel.findByIdAndUpdate(idTienda, { $pull: { arrAnimalitos: compra.idAnimalito } }, { new: true });
            if (rmvAnimalito == null) {
                return res.status(400).send({
                    estatus: '400',
                    err: true,
                    msg: 'Error: no se pudo quitar el animalito del array de animalitos.',
                    cont: 0
                });
            }
